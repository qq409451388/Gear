...
class EzCurl2
{
    ...
    public function setUserAgentFromApple() {
        $this->setUserAgent("Mozilla/5.0 (iPhone; CPU iPhone OS 6_1_4 like Mac OS X) AppleWebKit/536.26 (KHTML, like Gecko) Version/7.0 Mobile/10B350 Safari/9537.53");
        return $this;
    }

    public function setUserAgentFromChrome() {
        $this->setUserAgent("Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/36.0.1985.143 Safari/537.36");
        return $this;
    }
    ...
    private function buildResponse(string $httpMethod, string $res):EzCurlResponse {
        ...
        $responseObj->contentType = is_null($responseObj->responseHeader)?null:$responseObj->responseHeader->contentType;
    ...
    private function geneRequestMsg(EzCurlResponse $response):string {
        ...
        if($this->isDebug){
            ...
            $msg .= print_r(is_null($response->responseHeader)?"":$response->responseHeader->toString(), true).PHP_EOL;
