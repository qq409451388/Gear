...
class EzDataUtils
{
    ...
    public static function isMap($array, $keyTypeExpect, $valueTypeExpect)
    {
        ...
        foreach ($array as $k => $v) {
            $keyType = gettype($k);
            DBC::assertTrue(
                EzDataUtils::dataTypeNameEquals($keyType, $keyTypeExpect),
                "[EzObject] Match data Fail! The Map Key " . EzDataUtils::toString(
                    $k
            ...
            $valueType = gettype($v);
            DBC::assertTrue(
                EzDataUtils::dataTypeNameEquals($valueType, $valueTypeExpect),
                "[EzObject] Match data Fail! The Map Value " . EzDataUtils::toString(
                    $v
