...
class PHPExcel_Writer_HTML implements PHPExcel_Writer_IWriter {
    ...
    public function toHtmlData() {
        ...
        // add headers
        $htmlStr .= $this->generateHTMLHeader(!$this->_useInlineCss);

        // add navigation (tabs)
        if ((!$this->_isPdf) && ($this->_generateSheetNavigationBlock)) {
            $htmlStr .= $this->generateNavigation();
        }

        // add data
        $htmlStr .= $this->generateSheetData();

        // add footer
        $htmlStr .= $this->generateHTMLFooter();
